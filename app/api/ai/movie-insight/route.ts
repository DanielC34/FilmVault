...
export async function POST(req: Request) {
    try {
        ...
        return NextResponse.json({ insight: response.text || 'A modern masterpiece that defies convention.' });
    } catch (error: any) {
        console.error('Gemini movie-insight error:', error);
        return NextResponse.json(
            { insight: 'An essential piece of cinematic history.' },
            { status: 200 },
        );
    }
}
